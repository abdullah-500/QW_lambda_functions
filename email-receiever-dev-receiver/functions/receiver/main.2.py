...
def getSummary(email_text):
    tse_url = 'http://172.31.19.11:9094/tse'
        ...
        for device in devicesIds:
            if "app_platform" in device and device["app_platform"]==app_platform:
                if app_platform == 'ios' and user["iOS"]["send_expired_notification"] == True:
                    pushNotification.send(user["username"], "Expired", "Subscription Has been Expired, Please subscribe", "", device["gcm_endpoint"],False)
...
def is_primary_account_active(primary_email):
    ...
    try:
        ...
        #response = boto3.resource('dynamodb').Table("devices-dev").query(IndexName='user_emails_index',KeyConditionExpression=Key('user_emails').eq(primary_email))
    except Exception as e:
        print_log(e)
...
def lambda_handler(event, context):
    ...
    isSub = getClientIdByEmail(email_model_data["to"])
    print_log("======primary_email on secondary_email isSub =========",isSub)
    ...
    if platformEndpointArn:
        ...
        for platformEndpoint in platformEndpointArn:
            ...
            if platformEndpoint not in skipplatformEndpoints:
                ...
                pushNotification.send(email_model_data['from_username'], email_model_data['subject'], email_model_data['summary'], messageId, platformEndpoint,True)
    return